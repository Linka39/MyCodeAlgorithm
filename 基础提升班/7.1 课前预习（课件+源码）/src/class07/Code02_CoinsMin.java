...
/**
 * 换钱的最少货币数
 * 【题目】 给定数组 arr，arr 中所有的值都为正数且不重复。每个值代表一种面值的货币，每种面值的货币可以使用任意张，再给定一个整数 aim，代表要找的钱数，求组成 aim 的最少货币 数。
 * 【举例】 arr=[5,2,3]，aim=20。 4 张 5 元可以组成 20 元，其他的找钱方案都要使用更多张的货币，所以返回 4。
 * 	arr=[5,2,3]，aim=0。 不用任何货币就可以组成 0 元，返回 0。
 * 	arr=[3,5]，aim=2。 根本无法组成 2 元，
 * 	钱不能找开的情况下默认返回-1。
 */
public class Code02_CoinsMin {
	...
	public static int process(int[] arr, int i, int rest) {
		...
		for (int k = 0; k * arr[i] <= rest; k++) {
			...
			if (next != -1) { // 说明这个后续过程有效
				res = res == -1 ? next + k : Math.min(res, next + k); // 为当前k张钱有效的张数赋值
			}
		}
		return res;
	}
	...
	public static int minCoins2(int[] arr, int aim) {
		...
		// 设置最后一排的值，除了dp[N][0]为0之外，其他都是-1，代表此时剩余的钱不是0
			...
			for (int rest = 0; rest <= aim; rest++) { // 每一行都从左往右
				...
				if (rest - arr[i] >= 0 && dp[i][rest - arr[i]] != -1) {
					if (dp[i][rest] == -1) { // 如果之前下面的值无效
						dp[i][rest] = dp[i][rest - arr[i]] + 1;  // 找零钱方式+1
	 ...
	 * @return
	 */
	public static int coinKinds(int[] arr, int i, int rest, int aim){
		if (rest == 0) {
			return 1;
		}
		if (rest < 0) {
		...
		int num = 0;
		for (int j = i; j < arr.length && rest > 0 ; j++) {
			num += coinKinds(arr, j, rest - arr[j], aim);
		}
		return num;
	}
